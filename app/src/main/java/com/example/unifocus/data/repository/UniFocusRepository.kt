...
class UniFocusRepository(private val database: UniFocusDatabase) {
    ...
    fun selectTask(name:String, value:Boolean) {
    ...
    fun getTasksByGroup(group:String) : List<Task> {
        return taskDao.getTasksByGroup(group)
    }
    fun getTodaySelectedTasks(start: Long, end: Long): Flow<List<Task>>{
